...
const channelsToAdd = {
  education: [
    ...
    { id: 'UCqovsNSzYPkUzsMX0qNwbYg', name: '予備校のノリで学ぶ「大学の数学・物理」' },
  ],
  cooking: [
...
async function getChannelInfo(youtubeChannelId) {
  try {
    ...
    console.error(`Error fetching ${youtubeChannelId}:`, error.message);
    return null;
  }
}
  ...
  for (const [category, channels] of Object.entries(channelsToAdd)) {
    ...
    for (const ch of channels) {
      ...
      if (info) {
        ...
        if (success) {
          console.log(`  ✅ 追加: ${info.title} (${info.subscriberCount.toLocaleString()}人)`);
          addedCount++;
        } else {
          console.log(`  ❌ 追加失敗`);
          errorCount++;
