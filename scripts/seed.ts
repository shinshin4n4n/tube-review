...
async function executeSQLViaPostgREST(sql: string): Promise<void> {
  // PostgreSQL REST API経由でSQLを実行
  // 注: これは直接的なSQL実行ではなく、データ挿入のみ可能

  console.log(
    "ℹ️  Using Supabase SQL Editor is recommended for seed.sql execution\n"
  );
...
async function runSeed() {
  ...
  console.log(
    `📄 Loaded: supabase/seed.sql (${(seedSQL.length / 1024).toFixed(2)} KB)\n`
  );
