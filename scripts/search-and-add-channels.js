...
async function addChannel(category, channelInfo) {
  // 既存チェック
...
(async () => {
  ...
  for (const [category, queries] of Object.entries(searchQueries)) {
    ...
    if (needed <= 0) {
      console.log(`  ✅ 既に${targetPerCategory}チャンネル以上登録されています`);
    ...
    for (let i = 0; i < maxAttempts && added < needed; i++) {
      ...
      for (const ch of channels) {
        ...
        if (details && details.subscriberCount >= 10000) { // 最低1万人登録者
          const result = await addChannel(category, details);

          if (result.success) {
            console.log(`    ✅ ${details.title} (${details.subscriberCount.toLocaleString()}人)`);
            added++;
  ...
  for (const [category, result] of Object.entries(results)) {
    ...
    console.log(`${status} ${category}: ${result.added}/${result.needed}チャンネル追加`);
  }
})();
