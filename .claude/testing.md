...
3. `npm run test:unit` で **FAIL を確認**してから次へ

### 🟢 GREEN: テストを通す最小限の実装

1. テストを通す **最小限** のコードを書く
2. `npm run test:unit` で **PASS を確認**してから次へ

### 🔵 REFACTOR: テストを維持しながら改善

1. 重複除去、命名改善、責務分離
2. `npm run test:unit` で **PASS を維持**

### TDD ルール

- テストを先に書いてから実装する（テスト未記述の機能追加は禁止）
- 1サイクルは小粒度で回す（大きな機能は複数サイクルに分割）
- `/tdd` コマンドで TDD サイクルを開始できる
